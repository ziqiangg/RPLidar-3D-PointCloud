...
# Hardware Settings
LIDAR_PORT = 'COM3'        # Change this if needed, or use 'auto' logic if preferred
...
NUM_STEPS = 9              # Number of vertical slices to take
...
def run_3d_scan():
    ...
    try:
        ...
        for i, servo_angle in enumerate(steps):
            ...
            # Capture robustness slice (2D data)
            current_slice = capture_robust_slice(lidar, f"Slice_{i}")
            
            # Convert to 3D and store
            # Coordinate System:
            # - Lidar spins in Vertical Plane (if mounted sideways) or Horizontal (if standard)
            # - Assumed Setup (based on typical 3D DIY): Lidar scans a circle, Servo tilts that circle.
            # - If Servo rotates the lidar around the HORIZONTAL axis (like a nodding head):
            #   Ref: elevation = servo_angle, azimuth = lidar_angle
            
            # Adopting 'xyzscan_servo_auto.py' logic:
            # - It seems to treat the Lidar scan as "Elevation" (vertical circle)
            # - And the Servo angle as the "Azimuth" (horizontal rotation of the base)
            # Let's stick to the standard spherical projection:
            
            # Using the specific projection math from xyzscan_servo_auto.py 
            # (assuming Servo is base, rotating Lidar unit horizontally)
            
            # "z_plane" in original code was the servo angle.
            azimuth_plane = servo_angle 
            
            for qual, elev_angle_lidar, r_mm in current_slice:
                r_m = r_mm / 1000.0
                
                # --- PROJECTION LOGIC START (Matches xyzscan_servo_auto.py) ---
                # This assumes the lidar is mounted vertically and spins 360 vertically
                # The servo rotates this assembly horizontally.
                
                # Normalize lidar angle logic
                effective_elev = elev_angle_lidar
                effective_azimuth_plane = azimuth_plane
                
                # If the lidar sees "behind" itself (180-360 in vertical spin), 
                # mapped to the opposite side of the azimuth plane
                if effective_elev > 180:
                    effective_elev = 360 - effective_elev
                    effective_azimuth_plane = (azimuth_plane + 180) % 360
                    
                theta_rad = math.radians(effective_azimuth_plane) # Horizontal angle
                phi_rad = math.radians(effective_elev)            # Vertical angle
                
                # Spherical -> Cartesian (Y-up)
                # h = horizontal distance from center axis
                h = r_m * math.cos(phi_rad)
                
                y = r_m * math.sin(phi_rad)       # Vertical Height
                x = h * math.cos(theta_rad)       # X (Left/Right)
                z = h * math.sin(theta_rad)       # Z (Depth)
                
                # Append to cloud
                point_cloud.append((x, y, z, r_m, qual))
