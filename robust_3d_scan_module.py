...
def _safe_lidar_cleanup(lidar: Optional[RPLidar]):
    """Best-effort LiDAR shutdown that never raises."""
    if not lidar:
        return

    for op_name, op in (
        ("stop", lambda: lidar.stop()),
        ("stop_motor", lambda: lidar.stop_motor()),
        ("disconnect", lambda: lidar.disconnect()),
    ):
        try:
            op()
        except Exception as e:
            print(f"[LiDAR] Cleanup warning during {op_name}: {e}")


def _reconnect_lidar(
    lidar: Optional[RPLidar],
    lidar_port: str,
    lidar_baudrate: int,
    lidar_pwm: int,
) -> RPLidar:
    """Reconnect LiDAR after transport-level faults (e.g., Errno 5)."""
    _safe_lidar_cleanup(lidar)
    time.sleep(0.4)

    new_lidar = RPLidar(lidar_port, baudrate=lidar_baudrate)
    try:
        new_lidar.stop_motor()
    except Exception:
        pass
    time.sleep(0.3)
    new_lidar.start_motor()
    new_lidar.motor_speed = lidar_pwm
    time.sleep(1.5)
    return new_lidar
...
def run_scan(
    ...
    if wait_for_step is None: wait_for_step = lambda x: "continue"
    ...
    all_points_3d = []
    generated_files = []
    final_points = np.empty((0, 5), dtype=np.float64)
    ...
    try:
        ...
        for i, servo_angle in enumerate(steps):
            ...
            if i > 0 and wait_for_step({
                'slice_index': i,
                'total_slices': len(steps),
                'next_angle': float(servo_angle),
                'next_servo_angle': float(servo_angle),
            }) == "stop":
                break
            ...
            if len(slice_points_2d) == 0:
                progress_callback({
                    'stage': 'warning',
                    'message': f'Empty LiDAR slice at index {i + 1}; attempting LiDAR reconnect',
                    'slice_index': i,
                    'total_slices': len(steps),
                })
                try:
                    lidar = _reconnect_lidar(
                        lidar,
                        lidar_port=lidar_port,
                        lidar_baudrate=DEFAULTS['lidar_baudrate'],
                        lidar_pwm=DEFAULTS['lidar_pwm'],
                    )
                except Exception as e:
                    raise RuntimeError(f"Failed to recover LiDAR after empty slice: {e}")
